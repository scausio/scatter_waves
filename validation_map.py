...
def get_cell_edges(centers):
    ...
    For pcolormesh with shading='flat', we need edge coordinates.
    This function converts N cell centers to N+1 edge coordinates.
    ...
    -------
    edges : numpy.ndarray
        1D array of cell edge coordinates (length = len(centers) + 1)
    """
    if len(centers) < 2:
        # If only one cell, create edges with half spacing
        dx = 0.5
        return np.array([centers[0] - dx, centers[0] + dx])
    
    # Calculate spacing (assume uniform or nearly uniform)
    dx = np.diff(centers).mean() / 2.0
    
    # Create edges: start at first_center - dx, end at last_center + dx
    edges = np.zeros(len(centers) + 1)
    edges[0] = centers[0] - dx
    edges[1:-1] = (centers[:-1] + centers[1:]) / 2.0  # Midpoints between centers
    edges[-1] = centers[-1] + dx
...
def plotMap(ds: xr.Dataset, variable: str, coast_resolution: str, 
    ...
    # Plot data
    # Convert cell centers to cell edges for pcolormesh compatibility
    # This ensures dimensions work with all matplotlib versions
    lon_edges = get_cell_edges(ds.longitude.values)
    lat_edges = get_cell_edges(ds.latitude.values)
    
    # Create meshgrid for edges
    # Note: meshgrid(X, Y) creates grids where X varies along columns, Y along rows
    # Result shape: (len(Y), len(X))
    lon_grid, lat_grid = np.meshgrid(lon_edges, lat_edges)
    x, y = m(lon_grid, lat_grid)
    
    # Get data as numpy array
    # The binning function returns transposed data: (longitude, latitude)
    # So var_data has shape (n_lon, n_lat) = (129, 97)
    # pcolormesh with edges (130, 98) expects data (129, 97) transposed to (97, 129)
    # which should match (n_lat, n_lon) for meshgrid(lon, lat)
    data_array = var_data.values if hasattr(var_data, 'values') else var_data
    
    # Check dimensions and transpose if needed
    if data_array.shape == (len(ds.longitude), len(ds.latitude)):
        # Data is (lon, lat), need to transpose to (lat, lon) for meshgrid
        data_array = data_array.T
    
    im = m.pcolormesh(x, y, data_array, 
                     cmap=config['cmap'],
                     vmin=config['vmin'],
                     vmax=config['vmax'],
                     alpha=0.8)
