...
for dataset in conf.experiments:
	...
	print (ds)
	print (ds.isel(obs=ds.hs>1).time)
	for mod in ds.model.values:
		outName = os.path.join(outdir, 'tracks_%s_%s.jpeg' % (mod, years))
		m = Basemap(llcrnrlon=np.nanmin(ds.longitude)-0.5, llcrnrlat=np.nanmin(ds.latitude)-0.5, urcrnrlat=np.nanmax(ds.latitude)+0.5, urcrnrlon=np.nanmax(ds.longitude)+0.5, resolution='f')
		m.drawcoastlines()
		...
		print (ds)
		# 	print (i)
		im = plt.scatter(ds.longitude,ds.latitude,c=ds['hs'],s=1,vmin=0,vmax=2,cmap='jet')
		plt.colorbar(im)
		plt.title('SWH [m]')
		plt.savefig(outName)
