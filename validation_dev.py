...
def main():
    ...
    for dataset in conf.experiments:
        ...
        sat_hs = ds.hs
        notValid = np.isnan(sat_hs.values)
        print (f'No. nans {np.sum(notValid)}: satellite')
        for mod in ds.model.values:
            ds_mod = ds.sel(model=mod)['model_hs']
            ds_mod = ds_mod.where(ds_mod > 0.25)
            notValid = notValid | np.isnan(ds_mod.values)
        print(f'No. nans {np.sum(notValid)}: sat and models')

        print(np.where( ~notValid))
        if conf.additional_filters.ntimes:
            ntimes = maskNtimes(ds.sel(model=dataset)['model_hs'].load().values, sat_hs.load().values, float(conf.additional_filters.ntimes))
            print (type(ntimes))
            print (type(notValid))
            print ()
            notValid = notValid | ntimes
            print(type(notValid))
            print(f'No. nans {np.sum(notValid)}: additional filter')
        for mod in ds.model.values:
            ds_mod = ds.sel(model=mod)['model_hs']
            outName=os.path.join(outdir, 'scatter_%s_%s.jpeg' % (mod,years))
            ...
            sat_val=sat_hs.values[~notValid]
            mod_val=ds_mod.values[~notValid]
            scatterPlot(sat_val,mod_val, outName, title=years.replace('_','-'),
                        xlabel=f'{conf.satName}',ylabel='Model SWH [m]')
            #scatterPlot(sat_hs.isel(obs=~notValid).load().values, ds_mod.isel(obs=~notValid).load().values, outName)# title=years.replace('_','-'),
                        #xlabel=f'{conf.satName}',ylabel='Model SWH [m]')
